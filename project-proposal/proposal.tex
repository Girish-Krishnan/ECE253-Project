...
The YOLOv9 detector~\cite{wang2024yolov9} extends the one-stage object detection family with an improved backbone–neck–head design featuring the \emph{Programmable Gradient Information Bottleneck (C2f-PGB)} and a \emph{Generalized Efficient Layer Aggregation Network (GELAN)} backbone (Figure \ref{fig:yolov9_arch}). Given an input image $I\in\mathbb{R}^{H\times W\times 3}$, the detector predicts $K$ bounding boxes $\{b_k\}_{k=1}^K$ and class confidence scores $\{s_k\}_{k=1}^K$ such that
\begin{equation}
...
As a second classical algorithm, we implement a Wiener filter–based motion deblurring algorithm following the public implementation of \cite{vladkarpushin2020}. This approach performs deconvolution in the frequency domain, with a closed-form solution that balances inverse filtering with noise suppression.
...
As a complementary approach to adaptive gamma correction, we will test the \textit{Dual Illumination Estimation} method by Zhang \textit{et al.}~\cite{zhang2019dualilluminationestimationrobust}, which performs simultaneous correction of both underexposed and overexposed regions using a pair of illumination maps. The algorithm models an image as $I = R \times L$, where $R$ is the reflectance and $L$ the illumination. For dark regions, it estimates $L$ directly using spatially adaptive smoothness priors, while for bright regions it processes the inverted image $I' = 1 - I$ and fuses both corrected results using multi-exposure image fusion. The final enhanced image is obtained as a locally weighted combination of forward and reverse corrected outputs:
\begin{equation}
I_{\text{final}} = \omega_f I'_f + \omega_r I'_r + (1 - \omega_f - \omega_r) I,
\end{equation}
where $\omega_f, \omega_r$ are exposure-dependent weights computed from contrast, saturation, and local brightness measures. 

This dual-path design enables balanced enhancement across varying lighting conditions. We will use the publicly available Python implementation by Attaiki~\cite{lowlightpython} to compare against the IAGCWD method. Despite being more computationally expensive, Dual Illumination Estimation can recover both over- and underexposed regions simultaneously. This makes it an interesting benchmark to compare against Adaptive Gamma Correction.
